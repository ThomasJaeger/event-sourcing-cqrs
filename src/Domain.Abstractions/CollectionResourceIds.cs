...
// publishing projection emits the same sentinel. The sentinel discipline applies
// to tenant-wide collections every authorized subscriber may watch; a collection
// scoped to one owning principal subscribes under the owner's id instead (ADR
// 0037). The dispatcher routes by exact ResourceKey equality, so the value is
// byte-identical on both sides and non-empty: the subscribe side rejects null or
// whitespace.
