...
public interface IQueryBus
{
    ...
    // authorization behavior and the ownership-filtering handlers read them. The bare overload above
    // keeps the no-principal path: ActorId empty, no roles, unenforced. Mirrors the command bus's
    // principal-carrying SendAsync overload, adapted for a read (no idempotency key, a query is
