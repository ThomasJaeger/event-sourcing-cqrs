...
public class PostgresMigrationRunnerTests : IClassFixture<PostgresFixture>
{
    ...
    public async Task First_run_applies_all_migrations_in_order_to_empty_database()
    {
        ...
        rows[4].Name.Should().Be("add_outbox_notify_trigger");
        rows[4].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        rows[5].Version.Should().Be(6);
        rows[5].Name.Should().Be("stream_id_to_text");
        rows[5].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        ...
        rows[8].Version.Should().Be(9);
        rows[8].Name.Should().Be("add_sku_to_inventory_id_lookup");
        rows[8].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        ...
        rows[11].Version.Should().Be(12);
        rows[11].Name.Should().Be("add_customer_summary");
        rows[11].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        rows[12].Version.Should().Be(13);
        rows[12].Name.Should().Be("add_inventory_dashboard");
        rows[12].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        rows[13].Version.Should().Be(14);
        rows[13].Name.Should().Be("add_order_detail");
        rows[13].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        rows[14].Version.Should().Be(15);
        rows[14].Name.Should().Be("add_current_user_roles");
        rows[14].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        rows[15].Version.Should().Be(16);
        rows[15].Name.Should().Be("add_events_tenant_id");
        rows[15].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        rows[16].Version.Should().Be(17);
        rows[16].Name.Should().Be("add_read_model_tenant_id");
        rows[16].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        rows[17].Version.Should().Be(18);
        rows[17].Name.Should().Be("tenant_scoped_sku_uniqueness");
        rows[17].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        rows[18].Version.Should().Be(19);
        rows[18].Name.Should().Be("tenant_scoped_command_idempotency");
        rows[18].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        ...
        rows[25].Version.Should().Be(26);
        rows[25].Name.Should().Be("tenant_scoped_inventory_dashboard_keys");
        rows[25].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        rows[26].Version.Should().Be(27);
        rows[26].Name.Should().Be("tenant_scoped_order_list_keys");
        rows[26].Checksum.Should().MatchRegex("^[0-9a-f]{64}$");
        ...
        log.Should().Contain("Applying 0012 add_customer_summary.");
        log.Should().Contain("Applying 0013 add_inventory_dashboard.");
        log.Should().Contain("Applying 0014 add_order_detail.");
        log.Should().Contain("Applying 0015 add_current_user_roles.");
        log.Should().Contain("Applying 0016 add_events_tenant_id.");
        log.Should().Contain("Applying 0017 add_read_model_tenant_id.");
        ...
        log.Should().Contain("Applied 27 migration(s).");
    }
    ...
    public async Task Re_run_against_already_applied_database_is_a_no_op()
    {
        ...
        (await ReadSchemaMigrationsAsync(connStr)).Should().HaveCount(27);
    ...
    public async Task Concurrent_runners_apply_the_migration_exactly_once()
    {
        ...
        // pending batch (0001 through 0027); the other sees nothing pending.
    ...
    public async Task Dry_run_reports_pending_without_touching_the_database()
    {
        ...
        log.Should().Contain("Dry run: 27 migration(s) pending.");
        ...
        log.Should().Contain(m => m.EndsWith("0012 add_customer_summary"));
        log.Should().Contain(m => m.EndsWith("0013 add_inventory_dashboard"));
        log.Should().Contain(m => m.EndsWith("0014 add_order_detail"));
        log.Should().Contain(m => m.EndsWith("0015 add_current_user_roles"));
        log.Should().Contain(m => m.EndsWith("0016 add_events_tenant_id"));
        log.Should().Contain(m => m.EndsWith("0017 add_read_model_tenant_id"));
