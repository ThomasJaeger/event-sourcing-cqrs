...
- grounds the following, reporting file:line and verbatim where structure matters:
  ...
  - **Commit-visibility ordering on the global position. This is the load-bearing item.** Sequence and identity values are handed out before commit on both engines, PostgreSQL sequences and SQL Server IDENTITY alike. A reader tailing `event_store.events` in ascending global position can therefore see N+1 while N's transaction is still open, and a reader that tracks a high-water mark skips N for good once N commits behind the mark. This is not a SQL Server novelty. Whether the Postgres design on disk today tolerates commit-order gaps or already suffers them is a disk question, and it gets answered before the SQL Server adapter copies the answer.

    The pre-flight enumerates every reader in the tree that reads `event_store.events` in position order or tracks a position high-water mark, and reports for each whether a late-committing row can be skipped. Verify rather than assume, and the list is a starting point rather than a closed set: `ProjectionStartupCatchUpService`, `ReadAllAsync` and each of its consumers, `IEventStoreHeadPosition` and the lag reader built on it, and the Replay Tool's rebuild read. The steady-state outbox drain is expected to come out immune, because a claim-based drain never reads by position at all, and that expectation gets confirmed rather than assumed.

    Note what TDD_RULES.md:120 already asks the contract suite to pin: global ordering by `global_position` is "monotonic and gap-tolerant as specified". Report what, if anything, on disk specifies it, and what the readers assume in the absence of a specification.

    The routing rule: if the pre-flight finds a reader where a gap loses an event, that is a finding about the Postgres design, not about SQL Server. It stops and surfaces to the loop before any SQL Server authoring begins, because the second adapter must not inherit it.
...
- **The contract suite's sharing mechanism.** TDD_RULES.md §5 requires one identical suite across four adapters, and disk appears to have none. Extracting it is the honest reading of the rule and it is also the thing that makes the four-peer claim checkable. How it is parameterized, where it lives, and what it demands of a backend are open.
  ...
  Hold one principle through the extraction: **a clean pass of the extracted suite against the second adapter is a hypothesis to check, not a result to celebrate.** A suite authored while only one engine existed can pass on a second engine by never asking the questions where the two engines differ. Green then means the suite is silent, and silence reads the same as agreement. Extraction therefore adds engine-divergence probes rather than only parameterizing the connection. The stress test this arc was reordered for is only run if the suite asks divergence questions.
...
Three divergence probes travel with the contract-suite fork on the same footing: session inputs the loop rules on, each reconciled against what the live engine does rather than against this list. They are the questions a suite written against one engine would not think to ask.

- **The clustered-index choice on the events table.** SQL Server clusters on the primary key by default. Clustering on a random Guid means page splits and fragmentation on the highest-write table in the system, and the row counts a Testcontainers suite reaches are far too small to show it. Candidate shape: cluster on the ascending position column and carry a nonclustered index on (stream_id, version). The probe is whether the schema or the suite says anything about this at all, because a default taken silently here is a production problem that no green bar reports.
- **Timestamp precision and ordering.** `timestamptz` and `datetime2` do not carry the same resolution. Any suite assertion on an exact timestamp, and any ordering by timestamp rather than by position, can pass on one engine and turn flaky on the other. The extracted suite encodes neither engine's precision.
- **Transient-error retry semantics**, beyond the unique-violation mapping the candidate shapes already carry. A PostgreSQL serialization failure (40001) and a SQL Server deadlock victim (1205) are different conditions with different retry characteristics. A shared retry policy that treats them as one is a decision to make and record, rather than a default to inherit from the adapter that happened to be written first.
