...
- Track A flag against Chapter 9's Order aggregate code block and Chapter 13's projection code, both of which reference `Guid` stream identifiers. Phase 17 reconciliation updates the depicted shape to `StreamId`. The pedagogical content of both chapters survives the reconciliation unchanged.
