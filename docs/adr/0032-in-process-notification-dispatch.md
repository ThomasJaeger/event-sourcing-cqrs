...
The retained backplane surface has a name. This ADR kept "the PostgreSQL LISTEN/NOTIFY backplane" without naming its types, which left the live implementation documented only in the superseded ADR 0027. The surface is `IHubBackplaneConnection` in `src/Application/SignalR/`, implemented by `PostgresHubBackplaneConnection` in `src/Infrastructure/SignalR/`. Hosts.Web registers the pair as a singleton, and `HubBackplaneHostedService` is its only consumer, holding the one LISTEN the decision above describes. A reader of this ADR should not have to open a superseded one to find the type it retained.

A teardown defect in that surface is fixed here. The subscription ran its listener on the caller's cancellation token. The listener parks in `WaitAsync` and exits only when that token is cancelled, so the enumerator's teardown, which awaited the listener, could not complete until the caller cancelled. A consumer that stopped iterating without cancelling blocked inside `DisposeAsync` for the remaining lifetime of its own token. The host's reader iterates to cancellation and never breaks, so the only consumer in the running system never reached the defect, and the tests that did paid it as a fixed cost they read as normal.

The rule is that a subscription owns the token its listener runs on. It links a token source to the caller's, so caller cancellation still propagates; it runs the listener on the owned token; and it cancels that source before awaiting the listener during teardown. Disposal completes whether or not the caller ever cancels. The owned source is disposed after the await on every path, and a listener exiting through the cancellation the teardown itself requested is a normal shutdown rather than an error.

ADR 0027 recorded that the backplane's shape parallels the outbox processor's listener: a dedicated long-lived connection, a `WaitAsync` loop, a notification wake, and reconnect on drop. That parallel held for the loop and not for the teardown. `OutboxProcessor` has owned a private cancellation source since it was written, cancelling it in `StopAsync` before awaiting its listener task. The backplane now does the same, adapted to a per-call subscription rather than a host-lifetime one, and the parallel holds on both halves.
