...
Accepted (May 2026). Superseded by ADR 0032 (June 2026).
...
- **Direct `IHubContext<DashboardHub>` injected into projection unit-of-work.** The unit-of-work would call the hub directly inside `CommitAsync`. Rejected: couples projections to SignalR specifically rather than to the publisher abstraction; violates the hexagonal layering rule, which `docs/ARCHITECTURE.md` records as Domain at the center with no I/O and Application depending on Domain and Domain.Abstractions and nothing else; requires projections to know about a transport mechanism they should be agnostic to. The `INotificationPublisher` abstraction was rejected too in this option, since `IHubContext` is itself a transport-aware interface; abstracting it would re-derive the same indirection the publisher port provides.
...
## Amendment: tenant-qualified hub groups (P10.7)

P10.7 closes the tenant case the P9.6 amendment left open. The hub group names are now tenant-qualified for the two production families, order and inventory, so a subscription is isolated to its tenant by the group name itself rather than only by the resource it names. The qualified form is `tenant:{tenant}:{prefix}:{resourceId}`, with the tenant rendered in the same compact N-format the stream id renders its tenant segment in (ADR 0011), so one tenant produces the same bytes wherever a group is built. The broadcast path and the subscribe path build the group through one Web-side construction site, so the two formats cannot drift; a broadcast that reached a different string than the one a subscriber joined would deliver to no one.

The broadcast side takes the tenant from the notification envelope, which gained the tenant in the preceding commit. The hub builds the broadcast group from the deserialized envelope alone, holding no per-connection state at that point, so the envelope tenant is load-bearing for routing: it is the only place the broadcast group's tenant segment can come from.

The subscribe side takes the tenant from the authoritative tenant the subscription-authorization endpoint now returns on an allow, not from the client-supplied group and not from a claim at the hub edge. The client continues to send the unqualified resource group; the hub parses it for the prefix and the resource id only and composes the tenant segment from the authorized tenant, so a client cannot name another tenant's group by choosing its group string. The endpoint returns the tenant as a bare GUID on the allow response, and the hub rewraps it to the typed tenant (ADR 0029) at the construction-site boundary, where an empty or malformed value is rejected. The deny response is unchanged: it carries no tenant and stays byte-identical across the not-authorized, not-owned, and not-found cases, so the existence-hiding the P9.6 amendment established is preserved, since a tenant only ever rides an allow.

The posture is fail-closed. An authorized result that carries no tenant is a contract violation, and the hub refuses it rather than joining an unqualified or default-tenant group. A subscription that cannot be tenant-qualified does not happen; the alternative would route a subscriber to a group no broadcast targets, or to a group another tenant's broadcast targets.

The tenant segment is present for every tenant, including the default, which diverges deliberately from the stream id. The stream id omits the segment for the default tenant so the persisted legacy corpus and every new default-tenant stream share one form (ADR 0011, and the isolation model in ADR 0031), a backward-compatibility concern that does not apply to a group string, which is ephemeral runtime state with no persisted history to stay compatible with. On a routing surface an absent segment meaning "the default tenant" would overload absence into a silent-default path, where a defect that dropped the tenant would read as a valid default-tenant group rather than as an error. The group is therefore always self-describing, and a missing qualifier is malformed rather than a default-tenant group.

Inventory is the load-bearing case. The subscription authorize for an inventory resource is a permission gate on ViewInventory with no tenant or ownership dimension, the same treatment the query side gives inventory as operational data with no owning customer. The qualified group is therefore the only mechanism that isolates one tenant's inventory subscription from another's, because the same sku is a legal key under two tenants once the SKU read models are tenant-scoped (ADR 0031), so two tenants subscribing to the same sku would otherwise share one unqualified `inventory:{sku}` group. That is why the cross-tenant inventory isolation is proven by a hub test rather than asserted. An order subscription stays isolated by the tenant-scoped order-detail read the ownership check already runs under, so qualifying the order group is for routing consistency rather than to close a leak; both families are qualified the same way so the broadcast and subscribe formats match across families.

Subscribe and unsubscribe operate on the same group. The hub caches the authorized tenant on the connection at a successful subscribe and reuses it to qualify the unsubscribe group through the same construction site, so leaving removes the group that joining added. Unsubscribe stays ungated and makes no authorization call, the posture the P9.6 amendment set; it reads the cached tenant, not the endpoint. A connection that never completed an authorized subscribe holds no cached tenant, so an unsubscribe on it removes nothing, since a membership never held is nothing to leave.

The reserved families stay reserved. The admin-metrics and customer-per-id groups have no production consumer in v1 and exist only as test inputs that prove an unknown prefix fails closed at the hub, so P10.7 qualifies the two real families and no others. The Phase 8 setup doc's locked group list named an admin-metrics form; the code carries no such production group, and this amendment records that the qualification covers order and inventory only.
