...
Chapter 13's `ch13_detailDeep_code` depicts `OrderDetailProjection` as a four-event relational projection (Sales-only) with no JSONB timeline. The chapter does not prescribe a timeline shape because it does not depict the timeline. Phase 6 leads the manuscript on this surface; Phase 17 reconciles.
...
- Type-token stability: the JSONB `type` field carries the same token `IEventTypeProvider` registers. If an event type rename ships in a future phase, the migration touches the timeline rows alongside the events table. Phase 15 (versioning) is the natural place to land this concern.
...
- The type-token resolution path on the query side becomes a coupling concern (changing the registered token for an event requires migrating the timeline rows). Phase 15's versioning work would re-evaluate.

The trigger conditions are query-driven or operational, not pedagogical. The pedagogical question (what shape Chapter 13 should depict) belongs to Phase 17 reconciliation, not to a trigger reversal here.
